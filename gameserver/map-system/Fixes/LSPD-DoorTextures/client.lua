...
addEventHandler ( 'onClientResourceStart', resourceRoot,
	function ( )
		...
		for i = 1, #property do
			...
			texture[ i ] = dxCreateTexture ( tostring( 'Fixes/LSPD-DoorTextures/textures/'.. property[ i ][ 1 ] ), 'argb', true, 'wrap', '2d' )
			if ( texture[ i ] ) then	
				
				if ( i <= 21 ) then
					
					shader[ i ], _ =  dxCreateShader ( 'Fixes/LSPD-DoorTextures/shader.fx', 0, 0, false, 'world' )
				else
					shader[ i ], _ =  dxCreateShader ( 'Fixes/LSPD-DoorTextures/shader.fx', 0, 0, false, 'object' )
				end
