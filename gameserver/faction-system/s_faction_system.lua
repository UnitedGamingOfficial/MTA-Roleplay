		...
		if (menuVisible==0) then
			...
			if (factionID~=-1) then
				...
				if query then
					...
					local theTeam = getPlayerTeam(source)
					triggerClientEvent(source, "showFactionMenu", getRootElement(), motd, memberUsernames, memberRanks, hasPlayerAccessOverFaction(source, factionID) and memberPerks or {}, memberLeaders, memberOnline, memberLastLogin, factionRanks,  factionWages, theTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations)
					--[[
					** Web Opimization project.
					local result = callRemote("http://127.0.0.1:88/mta.php?do=openFaction", openFactionCallback, 1, 1, false)
					if result then
						outputChatBox("sent")
					else
						outputChatBox("not sent")
					end]]
				end
			else
...
--[[ RETURNES FROM PHP (true, $motd, $note, $fnote, $factionRanks, $factionWages, $sortedPlayers, $sortedVehicles);
function openFactionCallback(isGood, motd, note, fnote, factionRanks, factionWages, sortedPlayers, sortedVehicles)
	outputChatBox(isGood)
	outputChatBox(motd)
end]]
