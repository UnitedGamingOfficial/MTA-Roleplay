...
function displayTaxiJob()
	removeEventHandler("onClientVehicleEnter", getRootElement(), startTaxiJob)
	blip = createBlip(-1995.6259765625, 149.0986328125, 27.679862976074, 0, 4, 255, 127, 255)
...
function startTaxiJob(thePlayer)
	if (thePlayer==localPlayer) then
		if (getElementModel(source)==438 or getElementModel(source)==420) then
			removeEventHandler("onClientVehicleEnter", getRootElement(), startTaxiJob)
			outputChatBox("You will be alerted when someone has ordered a taxi. Use /taxilight to toggle your taxi light.", 255, 194, 15)
			if (isElement(blip)) then
				destroyElement(blip)
			end
		end
