...
function replaceLightsWindow()
	...
	if not (wPaint) then
		...
		-- Create the window
		wPaint = guiCreateWindow(X, Y, Width, Height, "Headlight Colour Change", false )
			
		-- colour ID inputs
		iColour1 = guiCreateEdit( 0.2, 0.2, 0.6, 0.075, "", true, wPaint )
		lcol1 = guiCreateLabel( 0.22, 0.15, 0.6, 0.075, "Red Mixture:", true, wPaint )
		iColour2 = guiCreateEdit( 0.2, 0.4, 0.6, 0.075, "", true, wPaint )
		lcol2 = guiCreateLabel( 0.22, 0.35, 0.6, 0.075, "Green Mixture:", true, wPaint )
		iColour3 = guiCreateEdit( 0.2, 0.6, 0.6, 0.075, "", true, wPaint )
		lcol3 = guiCreateLabel( 0.22, 0.55, 0.6, 0.075, "Blue Mixture:", true, wPaint )
		
		-- Repaint
		bPaintSubmit = guiCreateButton( 0.0, 0.8, 0.42, 0.2, "Change", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintSubmit,  function(button, state)
			if(button == "left" and state == "up") then
				...
				if (col1 == "") or (col2 == "") or (col3 == "") then
					outputChatBox("You need to input all the colour IDs from 0-255.", 255, 0, 0)
				else					
		...
		-- Close
		bPaintClose = guiCreateButton( 0.5, 0.8, 0.42, 0.2, "Close", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintClose,  function(button, state)
			if(button == "left" and state == "up") then
