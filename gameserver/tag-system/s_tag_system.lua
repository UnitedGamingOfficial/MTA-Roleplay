...
function makeTagObject(cx, cy, cz, rot, interior, dimension)
	...
	if (tag~=9) then
		...
		exports['anticheat-system']:changeProtectedElementDataEx(obj, "creator", dbid, false)
...
function loadAllTags(res)
	...
	if (result) then
		...
		while run do
			...
			exports['anticheat-system']:changeProtectedElementDataEx(object, "creator", tonumber(row["creator"]))
			count = count + 1
			if id > highest then
				highest = id
			end
	...
	elseif getElementData(thePlayer, "tag") == 9 then
		...
		if (newTag>0) and (newTag<9) then
			...
			elseif (teamName~="East Seville Saints") and (newTag==8) then -- ESS
				...
				exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "tag", newTag, false)
