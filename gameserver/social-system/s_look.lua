	...
	function( targetPlayer, event )
		...
		for itemID, badge in pairs( exports['item-system']:getBadges() ) do
			if getElementData(source, badge[1]) then
				...
				--table.insert( look[7], badge[2]:sub( badge[2]:find( " " ) + 1 ) )
				table.insert( look[7], itemName )
				else
				table.insert( look[7], badge[2]:sub( badge[2]:find( " " ) + 1 ) .. " - " .. getElementData( source, badge[1] ) )
				end
			end
		end
