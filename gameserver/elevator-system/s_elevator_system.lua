...
function loadOneElevator(elevatorID)
...
function loadAllElevators(res)
	...
	local timerDelay = 500
	if (result) then
		while true do
			local row = mysql:fetch_assoc(result)
			if not row then break end
			--[[local co = coroutine.create(loadOneElevator)
			coroutine.resume(co, row.id, true)
			table.insert(threads, co)
			--loadOneElevator(row.id)]]
			setTimer(loadOneElevator, timerDelay, 1, row.id)
			timerDelay = timerDelay + 500
		end
		mysql:free_result(result)
		--setTimer(resumeCo, 1000, 4)
	end
end
...
function deleteElevator(thePlayer, commandName, id)
	if (exports.global:isPlayerAdmin(thePlayer)) or exports.donators:hasPlayerPerk(thePlayer, 14) or commandName == "PROPERTYCLEANUP" then
...
function delElevatorsFromInterior(thePlayer, commandName, intID)
	if (exports.global:isPlayerAdmin(thePlayer)) or commandName == "PROPERTYCLEANUP" then
