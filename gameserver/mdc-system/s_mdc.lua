...
function onGetVehicleDetails(vehiclePlate)
	local result = mysql:query_fetch_assoc("SELECT `id`, `model`, `color1`, `faction`, `color2`, `plate`, `owner`,`Impounded` FROM `vehicles` WHERE plate='" .. mysql:escape_string(vehiclePlate) .. "' LIMIT 1")
	-- send the details to the client
	if(result) then
		local tableresult = { }
		tableresult[1] = { }
		...
		if (tonumber(result["owner"])>0) then
			...
			for key, value in ipairs(exports.pool:getPoolElementsByType("team")) do
				local vfaction = tonumber(result["faction"])
			...
			tableresult[1][2] = factionName -- Fix for the "false" vehicles.
		end
