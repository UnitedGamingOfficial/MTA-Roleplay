		...
		if not id or not (tonumber(id)) then
			...
			local qWeimy = mysql:query("SELECT `faction_id` FROM `characters` WHERE `id` ='"..issuerCharID.."'")
				...
				if issuerFaction == 4 then
					issuerFaction = "Los Santos Towing & Recovery"
				elseif issuerFaction == 1 then
					...
					if takeFromBank > 0 then
						...
						exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "bankmoney", bankmoney - takeFromBank, false)
					end
				end
			end
		end
...
function issueTicket(thePlayer, theCommand, theVehicle, amount, ...)
	...
	if (logged==1) then
		...
		if (getTeamName(team) == "Los Santos Towing & Recovery") or (getTeamName(team) == "Los Santos Police Department") or (getTeamName(team) == "San Andreas State Police") then
