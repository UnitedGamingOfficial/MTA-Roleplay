...
local actionIcons =
{
	...
	[TAB_KEYS] = { 4, tocolor( 255, 255, 255, 50 ), "Keys" },
	[TAB_WEAPONS] = { -28, tocolor( 255, 255, 255, 50 ), "Weapons" },
...
local function getImage( itemID, itemValue )
	if itemID == 16 then -- Clothes
		...
		if not tmp or not tonumber(tmp) or tonumber(tmp) == 999 then
			return ":account-system/img/nil.png"
		else
			--return ":account/img/" .. ("%03d"):format(tonumber(itemValue)) .. ".png"
			return ":account-system/img/" .. tmp .. ".png"
		end
		...
		if not waitingForItemDrop then
			if button == "left" or ( button == "middle" and exports.global:isPlayerAdmin( getLocalPlayer( ) ) ) then
				if button == "left" and ( hoverItemSlot or clickItemSlot ) then
					...
					elseif state == "down" and ( getKeyState( "lshift" ) or getKeyState( "rshift" ) ) then -- quick drop
						state = "up"
						clickDown = 0
