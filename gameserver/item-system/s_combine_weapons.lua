		...
		if (itemCheck[1] == 115) then -- Weapon item
			...
			if (weaponAmmoCountValid > weaponSyncPacket[2]) then -- There has been shot with this gun
				...
				if firstClip ~= 0 then
					...
					updateItemValue(client, firstClipInventorySlot,firstClipArr[1] ..":"..weaponSyncPacket[3]  ..":"..  firstClipArr[3] )
					if tonumber(weaponSyncPacket[3]) == 0 then
						takeItemFromSlot(client, firstClipInventorySlot, false)
					end
				end
