...
function hasItem(element, itemID, itemValue, clothingTest)
	...
	if success then
		for key, value in pairs(saveditems[element]) do
			if clothingTest then
				if value[1] == 16 then
					if string.find(value[2], ":") then
						skin, clothingid = unpack(split(tostring(value[2]), ':'))
					else
						skin = value[2]
					end
					
					if tonumber( itemValue )==tonumber( skin ) then
						return true, key
					end
				end
			end
...
function deleteAll( itemID, itemValue )
	if itemID then
		-- populate temporary table with potential items to delete.
		local worldObjects = {}
		local fillTable = mysql:query("SELECT id FROM worlditems WHERE itemid = " .. itemID .. " AND itemvalue = '" .. mysql:escape_string( tostring( itemValue ) ) .. "'" )
		if fillTable then
			while true do
			r = mysql:fetch_assoc( fillTable )
			if not r then break end
				r.id = tonumber( r.id )
				table.insert( worldObjects, r.id )
			end
		end
		...
		-- delete remaining items in the world
