	...
	if itemID then
		...
		elseif badges[itemID] then
			if ( getElementData( source, badges[itemID][1] ) ) then
				...
				for key, badge in pairs ( badges ) do
					if key ~= itemID then
						if ( getElementData ( source, badge[1] ) ) then
							...
							if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 then
							exports.global:sendLocalMeAction( source, "removes a " .. badge[1] .. " from their head." )
							else
							exports.global:sendLocalMeAction( source, "removes a " .. badge[1] .. "." )
							end
						end
					end
				end
				...
				exports.global:sendLocalMeAction( source, "puts on a " .. badges[itemID][1] .. "." )
				end
			end
		...
		elseif itemID == 145 then
			...
			outputChatBox("Error J" .. ("%04d"):format(tonumber(itemID) or 9999) .. " - Report on http://unitedgaming.org/mantis/", source, 255, 0, 0)
		end
	end
end
