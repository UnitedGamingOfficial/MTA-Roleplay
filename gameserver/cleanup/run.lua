function doCleanUp(thePlayer)
	...
	local mQuery3 = mysql:query("select vehicles.id from vehicles left join characters on characters.`id` = vehicles.`owner` where (characters.lastlogin < ( NOW() - interval 14 day) or characters.cked != 0)")
	local count3 = 0
	if (mQuery3) then
		while true do
