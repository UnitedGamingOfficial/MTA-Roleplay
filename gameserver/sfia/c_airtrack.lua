			...
			if getVehicleEngineState(value) and model ~= 509 and model ~= 481 then
				...
				--if (isLineOfSightClear(px, py, pz, x, y, z, true, false, false, true, false, false, true, localPlayer)) then
					...
					if (tx) then
						...
						if(getVehicleType(value) == "Plane" or getVehicleType(value) == "Helicopter") then
							aircraft = true
						end
						...
						elseif(getVehicleType(value) == "Boat") then
							text = "Boat"
						else
							text = "Vehicle"
						end
...
local atcTowerSpheres = {
	...
	createColSphere (1867.0380859375, -2409.337890625, 65.824188232422, 10), --Los Santos airport
