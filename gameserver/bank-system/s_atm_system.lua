function createATM(thePlayer, commandName, withdraw, deposit)
	if (exports.global:isPlayerLeadAdmin(thePlayer) or exports.global:isPlayerScripter(thePlayer)) then
		if not (withdraw) or not (deposit) then
		...
		elseif (tonumber(withdraw)) and (tonumber(deposit)) then
			...
			if (tonumber(deposit) > 1) or (tonumber(deposit) < 0) then
				...
				if (id) then
					...
					exports['anticheat-system']:changeProtectedElementDataEx(object, "depositable", tonumber(deposit), false)
					exports['anticheat-system']:changeProtectedElementDataEx(object, "limit", tonumber(withdraw), false)
