...
function loadOneVehicle(id, hasCoroutine, loadDeletedOne)
	...
	if row then
		...
		if veh then
			...
			-- Vehicle Handling
			local vehHandling = fromJSON(row["handling"])
			for attribute, value in pairs(vehHandling) do
				setVehicleHandling(veh, attribute, value)
			end
			...
			-- element data
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "model", tonumber(row.model), true)
			...
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "description:1", row.description1, true)
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "description:2", row.description2, true)
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "description:3", row.description3, true)
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "description:4", row.description4, true)
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "description:5", row.description5, true)	
			-- impound shizzle
				...
				if engine == 0 then
					...
					if (vehKey) or (owner < 0) and (faction == -1) or (playerFaction == faction) and (faction ~= -1) or ((getElementData(source, "adminduty") or 0) == 1) then
						...
						elseif fuel >= 1 then
							...
							--------------------------------------------
							--Take the player's key -> Done by Anthony
							if exports['global']:hasItem(source, 3, vehID) and exports['global']:hasSpaceForItem(veh, 3, vehID) then
							...
							exports['pd-system']:addVehicleToScan( veh )
					...
					--------------------------------------------
					--Take the player's key -> Done by Anthony
					if exports['global']:hasItem(veh, 3, vehID) and exports['global']:hasSpaceForItem(source, 3, vehID) then
					exports['global']:takeItem(veh, 3, vehID)
					exports['global']:giveItem(source, 3, vehID)
					else
					end
					--------------------------------------------
					exports['pd-system']:removeVehicleFromScan( veh )
					setVehicleEngineState(veh, false)
					exports['anticheat-system']:changeProtectedElementDataEx(veh, "engine", 0, false)
				end
			end
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		local owner = getElementData(source, "owner")
		local faction = getElementData(source, "faction")
		local carName = exports.global:getVehicleRealName(source)
	...
	if (vfaction~=-1) then
		...
		if (faction~=vfaction) and (seat==0) then
			if (CanTowDriverEnter) then
				outputChatBox("(( This " .. exports.global:getVehicleRealName(source) .. " belongs to '" .. factionName .. "'. ))", thePlayer, 255, 194, 14)
