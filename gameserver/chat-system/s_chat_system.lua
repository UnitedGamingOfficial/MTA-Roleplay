		...
		elseif theChannel > 1 or radioID < 0 then
			...
			if radioID == -1 then
				local teams = {
					...
					getTeamFromName("Sanitary Andreas"),
					getTeamFromName("San Andreas State Police"),
					getTeamFromName("Los Santos Emergency Services"),
					getTeamFromName("Los Santos Towing & Recovery"),
					getTeamFromName("Government of Los Santos"),
					getTeamFromName("Los Santos International Airport"),
					getTeamFromName("San Andreas News Network"),
				}
...
function govAnnouncement(thePlayer, commandName, ...)
	...
	if (theTeam) then
		...
		if (teamID==1 or teamID==2 or teamID==3 or teamID==87 or teamID==45) then
			local message = table.concat({...}, " ")
...
function departmentradio(thePlayer, commandName, ...)
	...
	if (theTeam)  or (tollped) then
		...
		if (teamID==1 or teamID==2 or teamID==3 or teamID==4 or teamID == 15 or teamID == 25 or teamID == 71 or teamID == 36 or teamID == 64 or teamID == 87 or teamID == 45 or tollped) then
			if (...) then
				local message = table.concat({...}, " ")
				radio(thePlayer, -1, message)
...
function govooc(thePlayer, commandName, ...)
	...
	if (getTeamName(team) == "Los Santos Emergency Services") or (getTeamName(team) == "Los Santos Police Department") or (getTeamName(team) == "Government of Los Santos") or (getTeamName(team) == "San Andreas State Police") and (logged==1) then
		if not (...) then
...
function adminChat(thePlayer, commandName, ...)
	...
	if(logged==1) and (exports.global:isPlayerAdmin(thePlayer))  then
		if not (...) then
			...
			local players = exports.pool:getPoolElementsByType("player")
			local adminTitle = exports.global:getPlayerAdminTitle(thePlayer)
			local accountName = tostring(getElementData(thePlayer, "account:username"))
			for k, arrayPlayer in ipairs(players) do
				local logged = getElementData(arrayPlayer, "loggedin")
				local hidea = getElementData(arrayPlayer, "hidea")
				if(exports.global:isPlayerAdmin(arrayPlayer)) and (logged==1) and (not hidea or hidea == "false") then
					table.insert(affectedElements, arrayPlayer)
					outputChatBox("[A] "..adminTitle .. " " ..accountName..": " .. message, arrayPlayer,51, 255, 102)
	...
	if(logged==1) and (exports.global:isPlayerAdmin(thePlayer)) then
		if not (...) then
			...
			for k, arrayPlayer in ipairs(players) do
				...
				if (logged==1) then
					if exports.global:isPlayerAdmin(thePlayer) then
						outputChatBox("Admin Announcement: * " .. message .. " *", arrayPlayer, 255, 194, 14)
					end
				end
			end
			exports.global:sendMessageToAdmins("Adm: "..username.." made an announcement")
...
function showAdmins(thePlayer, commandName)
	...
	if(logged==1) then
		...
		local counter = 0
		admins = {}
		outputChatBox("Server Staff:", thePlayer, 255, 194, 14)
		for k, arrayPlayer in ipairs(players) do
		...
		for k, v in ipairs(admins) do
			...
			local hiddenAdmin = getElementData(arrayPlayer, "hiddenadmin")
			if hiddenAdmin == 0 or exports.global:isPlayerLeadAdmin(thePlayer) then
...
function gmChat(thePlayer, commandName, ...)
	...
	if logged == 1 and exports.global:isPlayerModerator(thePlayer) then
		if not (...) then
			outputChatBox("SYNTAX: /".. commandName .. " [Message]", thePlayer, 255, 194, 14)
		else
			local affectedElements = { }
			...
			for k, arrayPlayer in ipairs(players) do
				local logged = getElementData(arrayPlayer, "loggedin")
				if exports.global:isPlayerModerator(arrayPlayer) and logged == 1 then
					table.insert(affectedElements, arrayPlayer)
	...
	if (logged==1) then
		if(getElementData(thePlayer, "interview"))then
			if not(...)then
				...
				exports.global:giveMoney(getTeamFromName"San Andreas News Network", 200)
			end
		end
	end
end
