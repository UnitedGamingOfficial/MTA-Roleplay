			...
			if callerphoneIsTurnedOn == 0 then
				...
				elseif getElementData(thePlayer, "injuriedanimation") then
					...
					-- Otherwise find a fool to answer it
						...
						if bankMoney >= 1 then
							if not exports.donators:hasPlayerPerk(thePlayer, 6) and not exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "bankmoney", tonumber(bankMoney) - 1, false) then
						...
						if not reconning then 
							...
							exports.global:sendLocalMeAction(foundElement, "'s cellphone starts to ring.")
						end
						...
						setTimer(
							function( thePlayer )
								if isElement( thePlayer ) then
									...
									outputChatBox("[English] SMS from #7332 [#"..phoneNumber.."]: Thanks for your message. - San Andreas News Network", thePlayer, 120, 255, 80)
								end
							end, 3000, 1, thePlayer
						)
						local theTeam = getTeamFromName("San Andreas News Network")
