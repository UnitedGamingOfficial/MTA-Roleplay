				...
				for k, v in ipairs( value["spawnpoints"] ) do
					...
					if v["vehicle"] and isElement( v["vehicle"] ) and data then
						...
						if canPopulate2 then
							...
							local vehicle = createVehicle( getVehicleModelFromName(data[1]) or tonumber(data[1]), v[1], v[2], v[3], v[4], v[5], v[6], plate  )
							if not vehicle then
								...
								exports['anticheat-system']:changeProtectedElementDataEx(v["vehicle"], "carshop:taxcost", 2*(vehicleTaxes[getVehicleModelFromName(data[1])-399] or 25), false)
								exports['anticheat-system']:changeProtectedElementDataEx(v["vehicle"], "dbid", -1, true)
								for i = 1, 5, 1 do
									exports['anticheat-system']:changeProtectedElementDataEx(v["vehicle"], "description:" .. i, "", true)
								end
