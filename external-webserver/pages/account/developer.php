 ...
 if (!isset($_SESSION['ucp_loggedin']) or !$_SESSION['ucp_loggedin'])
{
 ...
 # No worries, we go a step further in the form to authorize the level set, because Chuevo cares about security.

 $f_password = "fkjsdfkljsdaklfjasdlkfjasdklfjsdklajfklsdjf";
  if (!$_SESSION['ucp_username'] == $codeblue){
	 	 Header ("Location: /logout/");
 }

if($_POST['remove-secure'] == "Remove Developer Mode"){
	
	$rquery = mysql_query("SELECT `admin` FROM `accounts` WHERE `id`='" . $_SESSION['ucp_userid'] . "' LIMIT 1", $MySQLConn);
	mysql_query("UPDATE `accounts` SET `admin`='0' WHERE `id`='".$_SESSION['ucp_userid']."' LIMIT 1", $MySQLConn);
	setcookie("devmode", "0");
	header("Location: /account/dev/");
}
 ...
 ?>
       ...
       <? if($adminLevel == "8"){?> <h1 class="cheeky">Active</h1><br/>
	   <form name="removedev" action="" method="post">
       <input type="submit" class="button link" name="remove-secure" value="Remove Developer Mode"/>
       </form>
	   <?php }else{ ?>
